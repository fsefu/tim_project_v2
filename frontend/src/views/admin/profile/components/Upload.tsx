  ...
  const handleSubmit = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        axios
          .post(
            "http://127.0.0.1:8000/news-letter/upload-newsletter-pdf/",
            formData,
            {
              withCredentials: true,
              headers: {
          ...
          .catch((error) => {
            setIsLoading(false);
            toast.error("PDF Upload Error:", error);
          });
      })
      .catch((err) => {
        setIsLoading(false);
          ...
          >
            ...
            {filePreview ? (
              ...
              >
                ...
                <div className="ms-3 text-sm font-normal">{filePreview}</div>
                <button
                  type="button"
              ...
              // <Document file={filePreview} width={200} height={300}>
              //   <Page pageNumber={1} />
              // </Document>
              // <img
              //   src={filePreview}
          ...
          {filePreview ? (
            ...
            <>
              <p className="leading-1 mt-2 text-base font-normal text-gray-600">
                Upload a PDF file to extract text and use OpenAI's API to
                generate informative articles and news based on the content of
                the PDF.
              </p>
            </>
          )}
...
//         </h5>
//         <p className="leading-1 mt-2 text-base font-normal text-gray-600">
//           Upload a PDF file to extract text and use OpenAI's API to generate
//           informative articles and news based on the content of the PDF.
//         </p>
//         <button
//           onClick={handleSubmit}
//           className="linear mt-4 flex items-center justify-center rounded-xl bg-brand-500 px-2 py-2 text-base font-medium text-white transition duration-200 hover:bg-brand-600 active:bg-brand-700 dark:bg-brand-400 dark:text-white dark:hover:bg-brand-300 dark:active:bg-brand-200"
